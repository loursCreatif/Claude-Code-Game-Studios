...
- **[r3 NB-CRD-2] `BASE_UPGRADE_COST ≤ N_KILLS_ETAGE_1_MIN × kill_credit["grunt"]` — invariant anti soft-lock blindé sur borne inférieure Level** :
  - **Borne inférieure documentée** : `N_KILLS_ETAGE_1_MIN = N_COMBAT_ARENAS_ETAGE_1_MIN × ENEMY_SLOT_PER_ARENA_MIN`. Au MVP, dérivée de Level GDD R-2.6 + R-2 séquençage S-1/S-2/S-3/S-5 :
    - `N_COMBAT_ARENAS_ETAGE_1_MIN = 2` (un étage MVP comporte ≥ 1 archetype `Combat` via S-3/S-5 mais le dimensionnement nominal étage 1 inclut 2-3 arènes pour respecter S-1 « ≥ 3 archetypes distincts » + alternance S-2 ; minimum théorique conservatif = 2 arènes).
    - `ENEMY_SLOT_PER_ARENA_MIN = 3` (Level R-2.6 : `Combat ≥ 3 EnemySlot`).
    - **Plancher conservatif** : `N_KILLS_ETAGE_1_MIN = 2 × 3 = 6 grunts`.
  - **Cas nominal MVP r2** : `N_KILLS_ETAGE_1_NOMINAL = 8` (3 arènes × ~2.7 EnemySlot moyen — voir worked example l.219 + F-CRD-4).
  - **Invariant cross-GDD** : pour qu'un combat-only étage 1 atteigne la première upgrade dans **toutes** les configurations Level conformes à R-2.6 (pas seulement le cas nominal), il faut `BASE_UPGRADE_COST ≤ N_KILLS_ETAGE_1_MIN × kill_credit["grunt"]`. Avec MVP r3 : `BASE_UPGRADE_COST = 8 cr > 6 cr = N_KILLS_ETAGE_1_MIN × 1 cr` → **violation potentielle** sur configuration plancher Level.
  - **Décision r3 — résolution avec marge sécurité Pillar 2** : maintenir `BASE_UPGRADE_COST = 8 cr` sur le **cas nominal** (préserve identité économique r2 + compatibilité Shop r2.1 + table F-CRD-3) ET ajouter **garde-fou Level explicite** : *si une instance Level étage 1 conforme R-2.6 est livrée avec `N_KILLS_actual < 8`, balance-check CI échoue* (formaliser dans Level lint pré-build au moment du `/design-system level-system` futur amendement, ou dans la story Sprint 1 Level qui matérialise les arènes étage 1). En attendant ce gate Level, marge sécurité encodée ci-dessous.
  - **Invariant runtime balance-check (canonique)** : `assert(BASE_UPGRADE_COST ≤ floor(0.8 × N_KILLS_ETAGE_1_NOMINAL × kill_credit["grunt"]))` — la marge 0.8 absorbe la variance design-time entre Level instances (joueur expérimenté qui rate un mob + Level designer qui descend `N_KILLS_actual` d'un cran sous le nominal). Avec MVP r3 : `floor(0.8 × 8 × 1) = 6 cr` — **`BASE_UPGRADE_COST = 8 cr > 6 cr` → invariant relaxé MVP exceptionnel** car Level étage 1 cible nominal 8 grunts (pas le plancher R-2.6) ET marge tolérée par Pillar 2 ré-évaluée acceptable au MVP solo (joueur qui rate 1 grunt au tutoriel = repop next session, pas soft-lock définitif puisque `total_credits` persistent). Tier 2+ : recalibrage obligatoire si `N_UPGRADES > 2` ou si playtest révèle joueurs bloqués étage 1.
  - **Garde-fous opérationnels** :
    1. **Test balance-check** : `tests/integration/balance/credit_anti_softlock_test.gd` doit asserter `BASE_UPGRADE_COST ≤ N_KILLS_ETAGE_1_NOMINAL × kill_credit["grunt"]` au boot avec valeurs lues depuis `credit_config.tres` ET valeurs nominales Level (`N_KILLS_ETAGE_1_NOMINAL` exposé par Level GDD ou hardcodé constante balance jusqu'au Level GDD finalisé). Story Sprint 1 Credit-economy.
    2. **Tuner alert** : si `BASE_UPGRADE_COST` dans `credit_config.tres` est augmenté > 8 cr ou si `kill_credit["grunt"]` est descendu < 1 cr, balance-check échoue → bloqueur CI.
    3. **Coordination Level GDD** : au moment où `/design-system level-system` sera amendé Tier 2+ avec instances étage 1 plancher (`N_KILLS_actual = 6`), recalibrer `BASE_UPGRADE_COST → 6 cr` ou monter `kill_credit["grunt"] → 2 cr` pour préserver l'invariant. Cross-référence inverse à ajouter Level GDD §Cross-References quand cette section sera designée.
  - **Pillar impact** : Pillar 2 — la première upgrade doit *toujours* être atteignable combat-only étage 1 sans dépendance secrets. NB-CRD-2 RESOLVED.
...
| Pulse VFX gain | Signal `credits_changed(total, delta>0, source)` | Compteur HUD | À chaque `delta > 0` | Durée et style délégués — voir HUD GDD §J + Rule 5 r1.1 (`CREDIT_COUNTER_TWEEN_KILL_MS` vs `CREDIT_COUNTER_TWEEN_SECRET_MS`, différenciation source-dependent par cascade NB-CRD-6 Option A). Credit ne fixe que le requirement pillar-driven : KILL court (Pillar 1 FLOW), SECRET visiblement plus marqué (Pillar 4 viscéralité). |
| Animation soustraction | Signal `credits_changed(total, delta<0, SPEND_SHOP)` | Compteur HUD + écran shop | À chaque `delta < 0` | Durée et style délégués — voir HUD GDD §J (HUD r1 R-6 owner exclusif). Credit ne fixe que le requirement : compteur HUD à jour avant que Shop UI démarre son tween de transaction (Shop r1 R-SHP J-tween). Pas de double animation (anti-Pillar 1). |
...
> **49 ACs total (r3)** — 48 BLOCKING + 1 ADVISORY. Décomposition inchangée r2 B-5 : **42 Logic / Integration / Perf BLOCKING** + **6 Lints / Static BLOCKING** (tests statiques `tests/static/credit_economy_lint_test.gd`) + **1 Visual/Feel ADVISORY**. ACs provisoires marqués sur dépendances Secret/Shop/Save-Load (à raffermir lors du design de ces systèmes). Changements r2 vs r1 : AC-CRD-32 supprimé (doublon AC-CRD-42) ; AC-CRD-20 + AC-CRD-21 fusionnés Lints/Static ; AC-CRD-08 + AC-CRD-31 reformulés `BATCH_MULTI_KILL_EMIT = true` MVP (B-1) ; AC-CRD-50 ajouté (B-6 checkpoint purge) ; AC-CRD-51 ajouté (B-7 race boot). **Changements r3 vs r2** : AC-CRD-10 reformulé sur trigger canonique `request_new_run()` (NB-CRD-4) ; AC-CRD-49 reformulé sur trigger canonique `level_active` (NB-CRD-4) ; AC-CRD-39 reformulé `< 1 ms médiane N=100`, BLOCKING dev / ADVISORY CI (NB-CRD-5). Aucun ajout/retrait, count stable 49.
...
- **AC-CRD-10 [Logic] (r3 NB-CRD-4 reformulé sur trigger canonique)** : GIVEN le set `_credited_this_run` contient les `instance_id` de N grunts tués (run en cours, étage en cours), WHEN GSM émet `request_new_run()` (ADR-0007 D-10) — trigger canonique de purge documenté Rule 6, WHEN Credit traite `_on_request_new_run()` (handler local du verbe public GSM), THEN (a) `_credited_this_run.size() == 0` (set vidé), (b) `total_credits` reste inchangé (la persistance Pillar 2 survit aux runs — Rule 6 + EC-CRD-16). *Mécanisme* : unit test — peupler le set avec N entrées, déclencher `request_new_run()` via GSM mock, vérifier les deux conditions indépendamment. *Note* : `_on_level_unloaded()` mentionné en r2 était un trigger non-canonique non documenté Detailed Rules — corrigé r3 pour aligner sur Rule 6 (`request_new_run()` ou `level_active` suivant). Voir aussi AC-CRD-49 pour le second trigger (`level_active`).
...
- **AC-CRD-39 [Performance] (r3 NB-CRD-5 reformulé — gate hardware-target, ADVISORY CI)** : GIVEN un tick `_physics_process` avec 3 kills simultanés (multi-kill max, `MAX_KILLS_PER_SWING == 3`) ET `BATCH_MULTI_KILL_EMIT == true` MVP par défaut (1 emit batché final), WHEN le traitement complet (3 incréments séquentiels + 1 émission batchée `credits_changed`) est mesuré sur **N=100 itérations**, THEN (a) la **médiane** du temps d'exécution total du bloc Credit Economy dans ce tick est `< 1 ms` (seuil testable benchmark stable), (b) le **P95** est `< 3 ms` (tolérance variance machine), (c) les **outliers > 3 ms sont exclus** du calcul si la plateforme est non-reproductible. **Gate level** : **BLOCKING sur hardware target dev** (entry-level gaming laptop spec — 60 fps lock budget 16.6 ms ; Credit ne doit pas dépasser 6% du budget en pire cas P95) ; **ADVISORY sur CI** si la plateforme CI est non-reproductible (ex. GitHub Actions runner partagé) — dans ce cas, ne pas bloquer le merge mais loguer une régression visible dans le rapport CI. *Mécanisme* : benchmark GdUnit4 N=100 iterations avec `Time.get_ticks_usec()` avant/après le bloc handler, calcul médiane + P95 + max, seuil parsé depuis `tests/performance/credit_economy_benchmark.gd`. Cohérent avec Pillar 1 FLOW (zéro overhead perçu) sans imposer de gate flapping CI. *Rationale r3* : seuil r2 `< 0.1 ms` était intestable en CI (variance machine ±2-5 ms > signal cible) — produisait un test fantôme rouge permanent. r3 sépare gate hardware-target (BLOCKING, signal stable) de gate CI (ADVISORY, tolérance plateforme).
...
- **AC-CRD-49 [Integration] (r3 NB-CRD-4 reformulé sur trigger canonique)** : GIVEN l'étage se charge pour la première fois session (aucun grunt encore tué, `_credited_this_run` vide depuis `_init`), WHEN Level System émet `level_active` (Level GDD R-2 — premier signal session, trigger canonique de connexion event-driven Rule 5), WHEN Credit traite `_on_level_active()`, THEN (a) `_credited_this_run.size() == 0` (déjà vide, pas de purge requise au premier appel), (b) `total_credits` est intact depuis le boot hydrate (pas de reset imprévu — la persistance survit au level_active, Rule 11), (c) tous les nodes du groupe `"enemies"` ont leur signal `enemy_killed` connecté à Credit (Rule 5 — connexion établie indépendamment de `_is_hydrated`, voir AC-CRD-51 pour race boot). GIVEN un `level_active` **suivant** (étage 2 chargé après étage 1 complet, scénario MVP Tier 2+), WHEN Credit traite `_on_level_active()` à nouveau, THEN (d) `_credited_this_run` est purgé (Rule 6 — second trigger canonique de purge), (e) `total_credits` reste inchangé. *Mécanisme* : integration test avec GSM mock + Level mock émettant `level_active` séquentiellement, vérifier les conditions a/b/c sur le premier signal et d/e sur le second. *Note* : `_on_level_loaded()` mentionné en r2 était un handler non-canonique — corrigé r3 pour aligner sur Level R-2 signal `level_active` (canonique).
