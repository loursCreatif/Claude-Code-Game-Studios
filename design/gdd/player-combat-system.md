...
> **Status**: **APPROVED r7 final** (2026-04-27 — amendement r7 cross-GDD résolution OQ-ENM-1 : `enemy_killed` migré de Combat-emitted vers **Enemy-emitted**. Combat reste consumer SYNC pour le trigger slow-mo (Rule 13). Modifications éditoriales : Rule 9 (multi-hit appelle `enemy.die()`, n'émet plus le signal), Interactions table (Enemy/Credit/VFX/Audio rows reformulées source = `Enemy.enemy_killed`), Published API (suppression du signal `enemy_killed`), Note pattern intra-tick (consommation re-écrite). Aucun changement architectural — résolution éditoriale du conflit cross-GDD identifié par Enemy review r1 §9. **Fresh `/design-review` r7 (game-designer fresh session, mode solo) verdict APPROVED 2026-04-27** — 10/10 emplacements vérifiés conformes, cross-GDD cohérent (Enemy r2 Rule 11.c + Audio r2.1 transparent + Credit r1 aligné), zéro résiduel claim Combat émetteur. r6 historique : APPROVED 2026-04-23 — propagation Addendum r5.2 achevée inline.)
> **Author**: Martin + design-system skill (auto mode, solo) — 7 revisions r1→r7, toutes tranchées et propagées inline. Historique détaillé (blocking counts par review, décisions Martin D1-D5 / M1-M2 / D-r3-1 / D-r3-2 / DEC-r5-1 / DEC-r5-2 / D-r4-1 / D-r4-2 / D-r4-3, faux positifs rejetés, spot-checks CLEAN) dans `design/gdd/reviews/player-combat-system-review-log.md`.
> **Last Updated**: 2026-04-27 (r6→r7 amendement OQ-ENM-1 — Enemy émet `enemy_killed`, Combat consumer)
...
> **Pending ADR** (r1 action, mis à jour r7): `adr-combat-tick-model.md` — ordre `_physics_process` parent/enfant, proprietaire cache `_prev_position`, interface MockMovement/MockEnemy, CONNECT_DEFERRED vs SYNC policy pour les **consumers Combat** (`swing_started`, `swing_ended`, `multi_kill`) **et la connexion Combat → `Enemy.enemy_killed` qui DOIT être SYNC** (frame-precise, requise pour slow-mo trigger Rule 13). A rediger par lead-programmer avant Sprint 1 Combat.
...
9. **Pénétration multi-ennemi (multi-hit)** : un sweep peut toucher plusieurs ennemis simultanément. Résolution :
   ...
   - **r7 amendement OQ-ENM-1** : Combat **n'émet pas** le signal `enemy_killed`. Combat appelle `enemy.die()` sur chaque collider hit. Le signal `enemy_killed(enemy: Node, position: Vector3)` est émis **par Enemy lui-même** depuis `die()` (Enemy GDD r2 Rule 11.c). Combat est consumer du signal (connection SYNC frame-precise, cf. Rule 13 slow-mo trigger). Cette résolution garantit que les futurs émetteurs de mort (Hazard System Tier 2+ qui appellent `enemy.die()` sans passer par Combat) déclenchent les mêmes consumers Credit/VFX/Audio sans bug architectural.
   - Si ≥ 2 kills dans le même tick : Combat émet `multi_kill(count: int)` (hook pour feedback audio/VFX dédié, pas MVP mais infrastructure prête). `multi_kill` reste **Combat-emitted** car il représente une corrélation au niveau swing (plusieurs Enemy.enemy_killed regroupés par Combat) — agrégation que seul Combat peut faire.
...
13. **Slow-mo au kill via `Engine.time_scale`** (r1 revise — decision Martin D3 + CD tranche BLOCKING #8 `_process` → `_physics_process` ; r7 amendement OQ-ENM-1 — source signal = Enemy) : à la réception du premier `enemy_killed` (émis par **Enemy** depuis `die()`, Enemy GDD r2 Rule 11.c, connexion **SYNC** frame-precise) d'un swing, Combat set `Engine.time_scale = SLOW_MO_SCALE = 0.3` (valeur r1) pendant `SLOW_MO_DURATION_MS = 50` (r1) wall-clock absolu via `Time.get_ticks_msec()`, puis restore `Engine.time_scale = 1.0`. 
...
17. **Mutual kill tick-meme — les deux meurent (r2 M1 Option C Hybrid, Martin 2026-04-23)** : si `Player.died()` est emis par Enemy System au **meme tick physique** ou Combat traite un `enemy_killed`, **les deux morts sont resolues**.
    ...
    **Sequence garantie** : 
    ...
    - Combat `_physics_process` (child, execute apres Movement) demarre. Si `_state == Swinging`, Combat execute son sweep normalement : resolution colliders du ShapeCast (Rule 9, multi-hit avec `MAX_KILLS_PER_SWING` et `_hit_this_swing`). Combat appelle `enemy.die()` sur chaque hit ; chaque Enemy emet son propre `enemy_killed` SYNC (r7).
...
| **Enemy System** (aval bidirectionnel — r7) | Receveur de `die()` au hit + **émetteur de `enemy_killed`** | Combat appelle `enemy.die()` sur les colliders Enemy retournés par le sweep (Rule 9). Enemy System est responsable de l'idempotence de son propre `die()` (Enemy GDD r2 Rule 6 + EC-ENM-11). **r7 amendement OQ-ENM-1** : Enemy émet `enemy_killed(enemy: Node, position: Vector3)` SYNC depuis `die()` (Enemy GDD r2 Rule 11.c). **Combat connecte `Enemy.enemy_killed` en SYNC** pour déclencher le slow-mo (Rule 13) — frame-precise requis. Combat n'émet plus ce signal (retiré de Published API r7). |
...
| **Credit Economy** (aval — r7 source migrated) | Convertit kills en crédits | **r7 amendement OQ-ENM-1** : Credit Economy connecte **`Enemy.enemy_killed`** (Enemy GDD r2 Rule 11.c) — pas `CombatSystem.enemy_killed` (retiré r7). Économie traite le crédit au kill, pas au swing — donc un swing miss n'affecte rien. Cette migration garantit que les kills par Hazard Tier 2+ (qui appellent `enemy.die()` sans passer par Combat) crédite naturellement le joueur. |
| **VFX & Feedback System** (aval — r7 source migrated pour kill) | Trail katana, impact flash, blood splash, slow-mo screen effects | Connecte les signals Combat (`swing_started`, `swing_ended`, `multi_kill`) + **`Enemy.enemy_killed`** (r7 amendement OQ-ENM-1 — pas `CombatSystem.enemy_killed` retiré). Connexions recommandées : `CONNECT_DEFERRED` pour consommateurs lourds (GPUParticles spawn, blood decal) — cohérent Movement CONNECT_DEFERRED pattern. VFX spawne le trail + blood splash, Combat ne crée aucun asset visuel lui-même. |
| **Audio System** (aval — r7 source migrated pour kill) | Swoosh swing, impact kill, slow-mo audio mix | Connecte signals Combat (`swing_started` → whoosh, `multi_kill` → layer audio optionnel) + **`Enemy.enemy_killed`** (r7 amendement OQ-ENM-1 → impact sec 50 ms, sample universel `combat_kill_clac` Audio GDD r2.1 Rule 13). `CONNECT_DEFERRED` recommandé pour les consumers Audio lourds. Le pitch audio n'est **pas** affecté par la slow-mo (cf. Rule 13). |
...
signal multi_kill(count: int)                                 # count >= 2, émis en sus des Enemy.enemy_killed individuels du même tick
```

> **r7 amendement OQ-ENM-1** — `enemy_killed(enemy: Node, position: Vector3)` est **émis par Enemy** (Enemy GDD r2 Rule 11.c), pas par CombatSystem. Combat est **consumer** de `Enemy.enemy_killed` (connection SYNC pour slow-mo Rule 13). Cross-references downstream (Credit/VFX/Audio/HUD) → tous se connectent à `Enemy.enemy_killed`, jamais à CombatSystem. Cette résolution adresse le conflit cross-GDD identifié par Enemy review r1 §9 et garantit qu'un kill par Hazard Tier 2+ (qui appelle `enemy.die()` hors Combat) déclenche les mêmes consumers sans bug architectural.

**Note pattern (cohérent ADR-0005, r7 mis à jour)** : signals directs depuis `CombatSystem` (child de Player), pas via EventBus. Emit depuis `_physics_process` uniquement. Typed payloads value-types (Vector3, int, Node). Consumer contract : handlers interdits de muter `CombatSystem._state` ou d'appeler `CombatSystem.attempt_swing()` de façon ré-entrante. **Ordre intra-tick** : `swing_started` précède les `Enemy.enemy_killed` du même tick (causalité : Combat appelle `enemy.die()` après avoir entré le state Swinging) ; `multi_kill` suit les `Enemy.enemy_killed` individuels du même tick (Combat les a tous reçus en SYNC pendant Rule 9, puis émet l'agrégation après le sweep) ; `swing_ended` émis au tick ACTIVE_TICKS+1.
...
|---|---|---|---|
| **Enemy System** (APPROVED r2 — MVP) | Hard (critique, **bidirectionnel r7**) | Combat appellera `enemy.die()` sur chaque collider Enemy retourné par le sweep. **Enemy émettra `enemy_killed(enemy: Node, position: Vector3)` SYNC** depuis `die()` (Enemy GDD r2 Rule 11.c). Combat est consumer SYNC pour slow-mo trigger (Rule 13). | **Enemy GDD r2 a livré** : (1) `die()` idempotent (Rule 6 + EC-ENM-11) ; (2) `is_dead() -> bool` getter (AC-ENM-07b) ; (3) `CollisionShape3D` layer=2 (Enemy) + LaserCone layer=3 (LAYER_ENEMY_HITBOX) ; (4) `r_enemy = 0.35 m` BoxShape MVP ; (5) **émission `enemy_killed` autoritative** depuis `die()` post-state mutation ATOMIC (Enemy r2 §Rule 11). |
| **Credit Economy** (Designed r1, MVP) | Soft (**source migrated r7**) | Connectera **`Enemy.enemy_killed(enemy, position)`** pour incrémenter les crédits — pas `CombatSystem.enemy_killed` (retiré r7). | **Credit Economy GDD DOIT** : (1) déterminer la valeur crédit par type d'ennemi (contract Credit ↔ Enemy, pas Combat — F-CRD-1 kill table) ; (2) consommer `Enemy.enemy_killed` sans muter `enemy` (read-only) ; (3) source = Enemy garantit que les kills Hazard Tier 2+ déclenchent aussi le credit (cohérence cross-source). |
| **VFX & Feedback System** (Not Started, MVP) | Soft (**source migrated r7 pour kill**) | Connectera `swing_started(direction)`, `swing_ended()`, `multi_kill(count)` (Combat-emitted) + **`Enemy.enemy_killed(enemy, position)`** (Enemy-emitted r7 amendement OQ-ENM-1). | **VFX GDD DOIT** : (1) connecter en `CONNECT_DEFERRED` pour consommateurs lourds (GPUParticles, décalques sang, post-process flash) ; (2) ne jamais muter `enemy` ou `player` via les handlers ; (3) spawner le trail katana pendant `swing_started` → `swing_ended` window ; (4) gérer le flash blanc 50 ms + splash sang au `Enemy.enemy_killed` (art-bible État 2). |
| **Audio System** (APPROVED r2.1 — MVP) | Soft (**source migrated r7 pour kill**) | Connectera signals Combat (`swing_started`, `multi_kill`) + **`Enemy.enemy_killed`** (Enemy-emitted r7) → sample universel `combat_kill_clac` per Audio GDD r2.1 Rule 13. | **Audio GDD r2.1 livre** : (1) connecter en `CONNECT_DEFERRED` ; (2) ne pas pitcher le son sous `Engine.time_scale` (AudioStreamPlayer default OK) ; (3) layer audio optionnel pour `multi_kill` (chord cyberpunk) — MVP sans, Tier 2+. **r7** : la migration source de `CombatSystem` → `Enemy` est transparente pour Audio GDD r2.1 (qui ne nommait pas la source explicitement). |
...
| **ADR-0015 Accessibility Interface Layer** | Accepted | Combat consomme `AccessibilityService` (autoload #5, D-1) via pull-pattern au `_ready()` + reconnect sur signal `settings_changed`. API consommée : `get_disable_slow_mo()`, `get_slow_mo_scale_mult()` [1.0, 3.33], `get_flash_mult()` [0.0, 1.0] (D-4 combat row). Clamping service-side (D-7) — Combat ne re-clampe pas. Tier coverage : Tier 1 = `reduce_motion` boolean + `disable_slow_mo` ; Tier 2+ = multipliers individuels via Settings Menu UI. Story-022 débloquée par cette ADR (TR-cmb-016 G-4). |
...
> **Source canonique accessibility (ADR-0015 D-1)** : les trois knobs ci-dessus (`reduce_motion_slow_mo_scale_mult`, `reduce_motion_disable_slow_mo`, `reduce_motion_flash_mult`) sont exposés par `AccessibilityService` via `get_slow_mo_scale_mult()`, `get_disable_slow_mo()`, `get_flash_mult()` respectivement. Combat consomme ces getters au `_ready()` + reconnect `settings_changed` (D-3 pull-pattern). Le clamping est service-side (D-7) — les valeurs reçues sont déjà clampées. Tier coverage : **Tier 1** = toggle `reduce_motion` global + `disable_slow_mo` booléen (MVP requis, accessibility floor) ; **Tier 2+** = multipliers granulaires via Settings Menu UI (ADR-0015 REQ-10 + OQ-ACC-3/4/5). Ne pas lire `OS.is_reduce_motion_enabled()` directement dans CombatSystem — forbidden pattern ADR-0015 D-9.
