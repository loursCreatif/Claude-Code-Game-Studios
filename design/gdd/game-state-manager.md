...
| **MenuSystem** APPROVED r2 / Implemented Sprint A (epic 13/13 closure 2026-05-02) | Hard | Lit `get_current_state()` pattern pull au `_ready` (R-MNU-1/3 + ADR-0007 D-9), écoute `state_changed` via CONNECT_DEFERRED (ADR-0007 D-10 + R-MNU-4 r2 BLK-1). Appelle 4 verbes MVP : `request_pause / request_resume / start_etage(1) / request_scene_transition(main_menu_path)`. Quit direct via `get_tree().quit()` (R-MNU-8) ; save-on-quit délégué SaveLoad `NOTIFICATION_WM_CLOSE_REQUEST` (R-SAV-9). Outbound-only zero couplage cross-system (R-MNU-18 lint static AC-MNU-50/57/63). |
