...
> **Décision 2026-04-28 — story-011 (buffer retry Option C) DÉPRÉCIÉE Tier 2+ revisit** : le pattern Option C (retry buffer côté Shop) requiert détecter l'échec `save_string_array` côté caller, mais ADR-0010 D-2 verrouille `save_string_array(...) -> void` + D-5 outbound-zero interdit `signal save_failed`. Implémenter Option C exigerait un amendement ADR-0010 r2 cascadant sur tout l'epic save_load. Coût/bénéfice MVP défavorable : ~80 lignes pour scénario combiné force-quit + disk full simultanés (~0.5%). **Risque résiduel EC-SHP-24 accepté MVP** sans buffer retry. Re-évaluer Tier 2+ si telemetry montre fréquence > 0.5% ou plaintes joueurs. Voir `production/epics/shop-system/story-011-saveload-buffer-retry-option-c.md` Completion Notes.
