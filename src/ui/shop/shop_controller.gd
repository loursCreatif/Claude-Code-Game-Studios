...
# Story-013 animations extraites dans shop_animations.gd (TD-008 split #1).
class_name ShopControllerScript
extends Control

# Shop animations helper — preload bypass class cache CI (pattern AudioCombatHandler).
const ShopAnimations := preload("res://src/ui/shop/shop_animations.gd")
...
# Animation helper — composition TD-008 split #1 (instancié dans _ready).
var _animations: ShopAnimations = null
...
func _ready() -> void:
	...
	# Story-013 — instancier le helper animations (TD-008 split #1).
	_animations = ShopAnimations.new()
	_animations._owner = self
	_animations._get_reduce_motion = Callable(self, "get_reduce_motion_for_test")
	_animations._animation_log = _animation_log
	_animations._shake_cooldown_until_ms = _shake_cooldown_until_ms
	_animations._set_displayed_credit_value = func(v: int) -> void: _displayed_credit_value = v
...
func _on_credits_changed(total: int, _delta: int, _source: int) -> void:
	_animations.animate_credit_counter(_displayed_credit_value, total)
...
func _on_buy_pressed(id: StringName, n_index: int) -> void:
	...
	if CreditEconomy.get_total() < cost:
		...
		_animations.animate_disabled_shake(id, n_index)
		return
	...
	_animations.animate_purchase_pulse(id, n_index)
